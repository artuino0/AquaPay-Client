...
const Header = () => {
  ...
  const pathName = (): JSX.Element => {
    ...
    if (location.pathname === "/services/list")
      return (
        <span>
          Servicios <span className="text-deep-blue">&gt;</span>{" "}
    ...
    if (location.pathname === "/catalogs/periods")
      return (
        <span>
          Catalagos <span className="text-deep-blue">&gt;</span>{" "}
          <span className="text-gray-500"> Periodos</span>
        </span>
      );
    ...
    if (location.pathname === "/catalogs/initialize")
      return (
        <span>
          Catalagos <span className="text-deep-blue">&gt;</span>{" "}
          <span className="text-gray-500"> Inicializar</span>
        </span>
      );
    if (location.pathname.includes("/catalogs/services"))
      return (
        <span>
          Catalagos <span className="text-deep-blue">&gt;</span> Cliente{" "}
          <span className="text-deep-blue">&gt;</span>{" "}
          <span className="text-gray-500"> Servicios</span>
        </span>
      );
    ...
    if (location.pathname.includes("/settings"))
      return <span>Ajustes generales</span>;
    return <></>;
  };

  return (
    <header
      className="bg-white flex items-center justify-between w-full px-6 text-sm animate__animated animate__fadeInDown animate__faster"
    ...
    >
      ...
      <div className="select-none grow font-medium cursor-pointer flex justify-center">
        <div className="text-gray-600 border rounded-full text-xs px-4 py-1 w-fit">
          {dateNow}
        </div>
      </div>
      <div className="select-none font-semibold flex gap-4 items-center w-[250px] justify-end">
        <div className="flex h-9 w-9 rounded-full bg-deep-blue items-center justify-center text-white text-lg">
          {name.charAt(0)}
        </div>
        <div>
